...
function StatusBadge() {
  const { t } = useLocale();
  return (
    <div className="bg-[rgba(16,185,129,0.15)] content-stretch flex gap-2 h-8 items-center justify-center px-4 relative rounded-2xl shrink-0 border border-[rgba(16,185,129,0.25)] shadow-[0px_0px_12px_0px_rgba(16,185,129,0.38)]">
      <div className="bg-[#10b981] rounded-[4px] shadow-[0px_0px_8px_0px_#10b981] shrink-0 size-2" />
      <p className="font-bold leading-[15.6px] not-italic relative shrink-0 text-[#10b981] text-[13px]">
        {t('connected')}
      </p>
    </div>
  );
...
export function NetworkSection({ networks, selectedNetwork, onNetworkChange }: NetworkSectionProps) {
  const { t } = useLocale();
  return (
    <div className="bg-[rgba(255,255,255,0.03)] relative rounded-[24px] shrink-0 w-full border border-[rgba(255,255,255,0.08)] shadow-[0px_8px_32px_0px_rgba(0,0,0,0.19)]">
      <div className="content-stretch flex flex-col gap-6 items-start p-7 relative w-full">
        <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
          <div className="content-stretch flex flex-col gap-1.5 items-start not-italic relative shrink-0">
