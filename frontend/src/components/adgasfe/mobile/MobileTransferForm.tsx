        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              {showTokenSelect && (
                <div className="absolute top-full mt-2 w-full bg-[#1e293b] border border-[rgba(255,255,255,0.08)] rounded-xl shadow-lg z-10 overflow-hidden max-h-72 overflow-y-auto">
                  {(['stablecoin', 'token'] as const).map(category => {
                    const group = availableTokens.filter(tk => tk.category === category);
                    if (group.length === 0) return null;
                    return (
                      <div key={category}>
                        <p className="px-4 pt-2.5 pb-1 text-[10px] font-bold uppercase tracking-wide text-[#64748b]">
                          {t(`category.${category}`)}
                        </p>
                        {group.map(token => (
                          <button
