...
| SDK | TypeScript, Viem, 자체 Relayer 기반 AdWallet SDK |
| 프론트엔드 | Next.js, wagmi, 기존 `Form1TransferUI` · `AdminDepositPage` 확장 |
| 인프라 | RPC, Bundler(또는 MEE), 광고 검증 서버, 포인트 백엔드(선택) |

---

## 1.4 기술 스택, 아키텍처 설계 결정 사항 및 구현 접근 방식

### 기술 스택

| 계층 | 기술 | 용도 |
|------|------|------|
| **스마트 컨트랙트** | Solidity ^0.8.20, OpenZeppelin, Hardhat | Paymaster, 스폰서 전송, 포인트·스왑풀 컨트랙트 |
| **계정 추상화** | EIP-4337 (EntryPoint, UserOperation, Paymaster) | 가스비 대납·포인트 결제의 표준 인터페이스 |
| **서명·트랜잭션** | EIP-712, Viem | 메타트랜잭션 서명 검증 및 체인 호출 |
| **SDK** | TypeScript, Viem, (Biconomy AbstractJS / 자체 Relayer) | 지갑·dApp에서 가스리스·포인트·스왑 호출 |
| **프론트엔드** | Next.js 15, React 19, wagmi, Tailwind CSS | 웹 데모·관리자·포인트/스왑 UI |
| **모바일 앱** | Capacitor 6, WebView + ad-gas.vercel.app | Android/iOS 네이티브 껍데기, AdMob 연동 |
| **지갑 연동** | WalletConnect, MetaMask, Rainbow Wallet | 다중 지갑·모바일 지원. Permit(가스 없는 approve)는 eth_signTypedData_v4 지원 지갑(MetaMask, Rainbow 등)에서만 가능 |
| **인프라** | 공용 RPC, Relayer 서버, (선택) Bundler/MEE | 트랜잭션 중계·광고 검증·포인트 mint |

### 아키텍처 설계 결정 사항

| 결정 사항 | 선택 | 이유 |
|-----------|------|------|
| **가스 대납 방식** | EIP-4337 Paymaster + 메타트랜잭션(서명 검증) | 표준화된 계정 추상화로 다체인·다지갑 호환; 형태1은 서명 기반 Relayer로 즉시 구현 가능 |
| **형태1 vs 형태2** | 형태1(즉시 무료) + 형태2(포인트) 병행 | 사용자 선택권 확대; 지갑/앱은 포인트 잔액·spendForGas만 연동하면 됨 |
| **지갑/앱 연동** | 컨트랙트 인터페이스(IAdWalletPoints 등) 공개, SDK는 선택 | SDK 의존 최소화; 컨트랙트 주소·ABI만으로 잔액·결제 연동 가능 |
| **광고 검증** | 오프체인 검증 후 서명/proof → Relayer 또는 컨트랙트 | 온체인에 광고 로직을 두지 않고, 검증된 결과만 proof로 전달해 보안·유연성 확보 |
| **스왑풀** | Uniswap V2 스타일 AMM, Paymaster로 스왑 가스 대납 | 단순한 x*y=k 모델로 감사·운영 부담 감소; 기존 Paymaster 정책만 확장 |
| **프론트·앱 분리** | 웹(Next.js) 단일 배포 + Capacitor WebView 앱 | 한 번 개발해 웹·앱 동시 제공; 앱은 네이티브 껍데기(AdMob·스토어)만 담당 |

### 구현 접근 방식

- **Phase 1**: 형태1 유지 + AdWalletPoints·Paymaster 연동 + 지갑/앱 연동 문서·ABI 공개  
  → 포인트 적립/결제와 “컨트랙트만 연동” 시나리오 검증.
- **Phase 2**: AdWalletSwapPool MVP + Paymaster로 스왑 가스 대납 + SDK/프론트 스왑·LP UI  
  → 무료 스왑·유동성 풀 동작 검증.
- **Phase 3**: 광고→스왑/포인트 통합 플로우, LP 대시보드, 보안·성능 정리  
  → 통합 플랫폼으로 확장.
- **코드·문서**: 기존 형태1 컨트랙트·SDK·UI는 유지하며, 신규(Points, SwapPool)는 별 모듈·스크립트로 추가해 점진적 통합.
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  사용자 / dApp / 지갑 앱 (Next.js 웹, Capacitor 앱, 외부 지갑)              │
└─────────────────────────────────────────────────────────────────────────┘
                                      │
                    ┌─────────────────┼─────────────────┐
                    ▼                 ▼                 ▼
            ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
            │  wagmi/지갑   │  │  GaslessSDK  │  │ IAdWallet*   │
            │  연결·서명    │  │  (형태1/2/   │  │ (balanceOf,  │
            │              │  │   스왑 API)  │  │  spendForGas)│
            └──────┬───────┘  └──────┬───────┘  └──────┬───────┘
                   │                │                 │
                   └────────────────┼─────────────────┘
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  오프체인: Relayer / 광고 검증 / (선택) Bundler·MEE                        │
│  - 광고 시청 검증 → proof 발급 → mint / 스폰서 트랜잭션 중계                │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
│  │ Paymaster         │ │ SponsoredTransfer│ │ (형태2)           │        │
│  └──────────────────┘ └──────────────────┘ └──────────────────┘        │
│  ┌──────────────────┐ ┌──────────────────┐                             │
...
| 컴포넌트 | 역할 |
|----------|------|
| **클라이언트(웹/앱)** | 지갑 연결, 서명 요청, 포인트·스왑 UI, 광고 시청 트리거 |
| **SDK** | 형태1 메타트랜잭션, (형태2) 포인트 조회·결제, (Phase2) 스왑·LP 호출 래핑 |
| **Relayer/광고 검증** | 광고 시청 검증, proof 발급, mint/스폰서 트랜잭션 전송 |
| **Paymaster** | UserOperation 검증·가스 대납; 필요 시 포인트 spendForGas 호출 |
| **AdWalletSponsoredTransfer** | EIP-712 서명 검증 후 네이티브/ERC20 전송(형태1) |
| **AdWalletPoints** | 포인트 잔액·mint·spendForGas(형태2·지갑 연동) |
| **AdWalletSwapPool** | 유동성·스왑(Phase2), 가스는 Paymaster 대납 |

### 데이터 흐름 요약

- **형태1(즉시 무료)**: 사용자 서명 → Relayer가 AdWalletSponsoredTransfer 호출 → Paymaster가 해당 트랜잭션 가스 부담.
- **형태2(포인트)**: 광고 시청 → 검증·proof → Relayer가 mint → 사용자 트랜잭션 시 Paymaster(또는 Executor)가 spendForGas 후 가스 대납.
- **스왑**: 유동성 풀에 예치/인출 또는 adProof(또는 포인트)와 함께 swap 호출 → Paymaster가 스왑 트랜잭션 가스 대납.
- **지갑/앱 연동**: balanceOf 조회 + UserOperation 제출 시 Paymaster가 spendForGas 호출 → 컨트랙트만으로 연동 완료.
...
- **설정**: `SdkConfig` — `publicClient`, `walletClient` (Biconomy 제거)
